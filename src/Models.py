...
class Solver(Base):
    ...
    fullname = column_property(name + "_" + version)
    ...
    def print_summary(self):
        ...
        print("Name: {} \nVersion: {} \nPath: {} \nFormat: {} \nProblems: {} \nCompetition: {} \nAuthor: {}".format(self.name,
                                                                                                                    self.version, self.path, self.format, 
                                                                                                                    self.get_supported_tasks(),self.competition,self.author))
    ...
    def run(self,task,benchmark,timeout):
        ...
        instances = benchmark.get_instances(self.format)

        if "DS" in task.symbol or "DC" in task.symbol:
            arg_lookup = benchmark.generate_additional_argument_lookup(self.format)
        
        for instance in instances:
            instance_name = Path(instance).stem
            params = [self.path,
                  "-p", task.symbol,
                  "-f", os.path.join(benchmark.path, instance),
                  "-fo", self.format]
            if arg_lookup:
            ...
            try:
                ...
                results[instance] = {'timed_out':False,'additional_argument': arg, 'runtime': run_time, 'result': solver_output}
            except subprocess.TimeoutExpired:
                results[instance] = {'timed_out':True,'additional_argument': arg, 'runtime': None, 'result': None}
...
class Benchmark(Base):
    ...
    def get_instances(self,extension,without_extension=False,full_path=False):
        instances = []
        for instance in os.listdir(self.path):
            if instance.endswith(extension):
                if without_extension:
                    instances.append(Path(instance).stem)
                elif full_path:
                    instances.append(os.path.join(self.path,instance))
                else:
    ...
    def generate_additional_argument_lookup(self, solver_format):
        lookup = {}
        argument_files = self.get_argument_files()
        for file in argument_files:
            argument_file_path = os.path.join(self.path, file)
        ...
        instances = self.get_instances(solver_format,without_extension=True)
    ...
    def generate_single_file(self, instance_name, generate_format, present_format):
        ...
        present_instance_path = os.path.join(self.path, "{}.{}".format(instance_name, present_format))
        ...
        generate_file = open(os.path.join(self.path, generate_file_name), 'a')
    ...
    def generate_single_argument_file(self, instance_name, present_format, extension="arg"):
        ...
        argument_file = open(os.path.join(self.path, argument_file_name), 'a')
    ...
    def generate_argument_files(self, extension="arg"):
        ...
        with click.progressbar(self.get_instances(self.get_formats()[0]),
             ...
             for file in files:
                ...
                if "{}.{}".format(name_extension[0], extension) not in os.listdir(self.path):
