...
def test(df: pd.DataFrame, cfg: config_handler.Config):
    ...
    if significance_cfg['non_parametric_test'] =='all' or 'all' in significance_cfg['non_parametric_test']:
            significance_cfg['non_parametric_test'] = register.non_parametric_significance_functions_dict.keys()
    results = {}
    for test in significance_cfg['non_parametric_test']:
        if test in register.non_parametric_significance_functions_dict.keys():
            _res = register.non_parametric_significance_functions_dict[test](df, cfg)
            results[test] = _res
    return results
...
def mannwhitneyu_test(df: pd.DataFrame, cfg: config_handler.Config):
    ...
    return df.groupby(['repetition','tag','task','benchmark_id'], as_index=False).apply(lambda _df: _mannwhitneyu(_df))
...
def kruskal_test(df: pd.DataFrame, cfg: config_handler.Config):
    ...
    return df.groupby(['repetition','tag','task','benchmark_id'], as_index=False).apply(lambda _df: _kruskal(_df))

register.non_parametric_significance_functions_register('mannwhitneyu',mannwhitneyu_test)
register.non_parametric_significance_functions_register('kruskal',kruskal_test)